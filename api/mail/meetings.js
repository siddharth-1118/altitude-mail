...
export default async function handler(req, res) {
  ...
  if (req.method === 'GET') {
    const { data, error } = await supabase.from('mail_meetings').select('*').order('created_at', { ascending: false });
  ...
  if (req.method === 'POST') {
    ...
    const { error } = await supabase.from('mail_meetings').upsert(meetings, { onConflict: 'id' });
    if (error) return res.status(500).json({ error: error.message });
